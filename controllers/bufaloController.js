...
const createBufalo = async (req, res) => {
    try{
        const {tagBufalo, idCriadouro, nome, idade, peso, raca, sexo, dataNasc, zootecnico, sanitario} = req.body;
        await bufaloService.Create(tagBufalo, idCriadouro, nome, idade, peso, raca, sexo, dataNasc, zootecnico, sanitario);
...
const updateBufalo = async (req, res) => {
    try{
        if (ObjectId.isValid(req.params.id)) {
            const id = req.params.id;
            const {tagBufalo, idCriadouro, nome, idade, peso, raca, sexo, dataNasc, zootecnico, sanitario} = req.body;
            const bufalo = await bufaloService.Update(id, tagBufalo, idCriadouro, nome, idade, peso, raca, sexo, dataNasc, zootecnico, sanitario);
